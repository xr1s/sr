...
impl GameData {
    pub fn item_config(&self, id: u32) -> Option<vo::misc::ItemConfig> {
        self._item_config().get(&id).map(|po| po.vo(self))
    }

    pub fn list_item_config(&self) -> Vec<vo::misc::ItemConfig> {
        self._item_config().values().map(|po| po.vo(self)).collect()
    }

    pub fn item_use_data(&self, id: u32) -> Option<vo::misc::ItemUseData> {
